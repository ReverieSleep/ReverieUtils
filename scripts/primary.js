...
function validateForm() {
    var btn = document.getElementById("validateForm");
	$('#container').css("filter","blur(2px)");
	$('#mypopup').css("display","block");
    btn.disabled = true;
    console.log($('#carrierSelection option:selected').attr('value'));
    if ($('#poNumber option:selected').attr('value') == '') {
    ...
    }/*else if($('#bol').val()=='' && $('#customer option:selected').attr('value') == 'Bloomingdales') {
		alert('BOL number is required');
		
	}*/else if ($('#itemNo option:selected').attr('value') == ''){
        alert('Item choice is required');
        /*else if(($('#carrierSelection option:selected').attr('value')=='addNewCarrier'
        || $('#carrierSelection option:selected').attr('value')=='' || $('#carrierSelection option:selected').attr('value')==0)
        && $('#customer option:selected').attr('value') == 'Bloomingdales') {
		...
		$.post('utils.php', {
			...
			upc: $('#itemNo option:selected').attr('value'),
			dcName: $('#dcName').val(),
            ...
            customer: $('#poNumber option:selected').attr('customer'),
            deptCode: $('#deptCode').val(),
			itemID: $('#itemNo option:selected').attr('itemid'),
            line_no: $('#itemNo option:selected').attr('line')
		},function(data) {
			console.log(data);
...
function createProduct() {
    if($('#description').val()=='') {
		...
		$.post('utils.php', {
			...
			upc: $('#upc').val(),
            customer: $('#customer option:selected').attr('value')
		},function(data) {
...
function populateDC() {
	if($('#dcPreset').val()!='') {
		...
		var dcCode = $('#dcPreset option:selected').attr('dccode');
		
		$('#dcName').val(dcName);
		$('#dcNumber').val(dcNumber);
		$('#dcAddress').val(dcAddress);
		
		//$('#storeNumberTextBox').val(dcCode);
		//$('#storeName').val(dcName);
...
function updateLabelCustomer(){
		var cust = $('#upc option:selected').attr('customer');
        $('#customer').val(cust);
}


function populateHeaderData() {
        var po_no = $('#poNumber option:selected').attr('order_no');
        var dept = $('#poNumber option:selected').attr('dept');
        var storeNo = $('#poNumber option:selected').attr('storeNo');
        var storeName = $('#poNumber option:selected').attr('storeName');
        $('#storeName').val(storeName);
        $('#storeNumberTextBox').val(storeNo);
        $('#deptCode').val(dept);
        $('#itemNo').empty();
        $.post('utils.php', {
            action: 'getLineItems',
            ...
            console.log(data);
            $('#itemNo').append(data);
            var initQty = $('#itemNo option:selected').attr('qty');
            //var initDesc = $('#itemNo option:selected').attr('itemdesc');
            $('#numLabels').val(initQty);
            //$('#itemDesc').val(initDesc);
        });
}

function updateQty(){
    var qty = $('#itemNo option:selected').attr('qty');
    //var desc = $('#itemNo option:selected').attr('itemdesc');
    $('#numLabels').val(qty);
    //$('#itemDesc').val(desc);
}

function populateStore() {
	if($('#storeNameSelect option:selected').html()=='') {
		$('#dcName').val('');
		$('#dcNumber').val('');
		$('#dcAddress').val('');
		$('#storeName').val('');
		$('#storeNumberTextBox').val('');
	}else{
		var dcName = $('#storeNameSelect option:selected').attr('dcName');
		var dcAddress = $('#storeNameSelect option:selected').attr('dcAddress');
		var dcNumber = $('#storeNameSelect option:selected').attr('dcNumber');
		var storeName = $('#storeNameSelect option:selected').attr('storeName');
		var storeNumber = $('#storeNameSelect option:selected').attr('storeNumber');
		
		$('#dcName').val(dcName);
		$('#dcNumber').val(dcNumber);
		$('#dcAddress').val(dcAddress);
		$('#storeName').val(storeName);
		$('#storeNumberTextBox').val(storeNumber);
	}
}
...
function viewLabelDetails(labelID) {
	...
	$.post('utils.php', {
		...
		window.open('/labels/label.php?labelID='+data,'_blank');
	});
}
...
function generateASN() {
	if($('#asn_controlNumberPadded').val()=='') {
		...
		var dc = $('#asn_dc').val();
		var store_no = $('#asn_storeNumber').val();
